...
def main():
    ...
    file_path = '1000questions.json'  # Replace with your JSON file path
...
# Accessing the structured data:
    ...
    num_correct = 0
    cnt = 0
    for question, data in question_data_pairs:
        cnt += 1
        query = question
        label = str(data) + ".txt"
        k = args.k
        ...
        for result in search_results:
            print (result['original_index'])
            if result['original_index'] == label:
                check = True
        if check:
            num_correct += 1
        
    print("PERCENTAGE", num_correct/250)
        # # Display the results
